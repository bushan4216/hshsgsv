...
if(isset($_POST['addtime'])) {
		...
		if(preg_match('/^\d{2}:\d{2}:\d{2}$/',$_POST['newtime'])) {
...
No. of Words: &nbsp;
<input name="nowords" type="text"  value="<?php echo $getNowords['NoWords'];?>" />&nbsp;&nbsp;<input name="setwords" type="submit" value="   Set   "/>
<br/></div>
...
echo "<div class='editTime'>Edit Duration Time :</div>";
...
$rowCount = 1;
	...
	while ($getValues = $frm->getArray($sqlEdit)) {
			...
			echo "<tr>
					...
					<td width='80'><input type='submit' name='updated' value = '  Save  ' /></td>
				</tr>";
	}
echo "</table>";
